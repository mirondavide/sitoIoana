...
exports.handler = async (event, context) => {
    ...
    console.log(`[${requestId}] update-product invoked: method=${event.httpMethod}`);
    ...
    // Find and replace product
    const productIndex = productsData.products.findIndex(p => p.id === product.id);
    if (productIndex === -1) {
        console.error(`[${requestId}] Product not found: ${product.id}`);
        return {
            statusCode: 404,
            body: JSON.stringify({ message: `Product with ID ${product.id} not found` })
        };
    }

    console.log(`[${requestId}] Updating product at index ${productIndex}`);
    ...
    const commitMessage = `Update product: ${product.name} (via admin panel)
